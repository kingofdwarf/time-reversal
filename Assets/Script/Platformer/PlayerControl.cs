...
public class PlayerControl : MonoBehaviour, IInputListener
{
	[SerializeField]
	...
	public void Move(float inputForce)
	{
		...
		anim.SetFloat("Speed", Mathf.Abs(inputForce));

		// If the player is changing direction (h has a different sign to velocity.x) or hasn't reached maxSpeed yet...
		if(inputForce * GetComponent<Rigidbody2D>().velocity.x < maxSpeed)
			// ... add a force to the player.
			GetComponent<Rigidbody2D>().AddForce(Vector2.right * inputForce * moveForce);
